...
parser.add_argument('--data_path', default='./data/citeulike-a_edgelist.npz',
...
parser.add_argument('--partition', default=2, type=int, help='Item_Matrix Partition.')
...
name2idx_item = {}
idx2name_item = {}
u_mtx = np.zeros((len(users), emb_size), dtype=np.float16)
i_mtx = np.zeros((len(items), emb_size), dtype=np.float16)
train_mask = np.ones((len(users), len(items)), dtype=np.float16)
...
for p in tqdm(range(args.partition + 1)):
    ...
    cur_u_mtx = torch.from_numpy(u_mtx[p*parts:min((p+1)*parts, len(u_mtx))]).to('cuda:0')
    cur_i_mtx = torch.from_numpy(i_mtx).to('cuda:0')
    cur_mask  = torch.from_numpy(train_mask[p*parts:min((p+1)*parts, len(u_mtx)), :])
    
    cur_dist_mtx = - torch.mm(cur_u_mtx, cur_i_mtx.t())
    cur_dist_mtx = cur_dist_mtx.to('cuda:0')  * cur_mask.to('cuda:0')
    cur_dist_mtx = cur_dist_mtx.to('cuda:0')
    cur_dist_mtx = torch.argsort(cur_dist_mtx, dim=1)[:, :top_k]
    cur_dist_mtx = cur_dist_mtx.cpu().numpy()
